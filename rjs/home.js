...
require(["service", "common", "jquery", "bootstrap"], function(service, common, $) {
	...
	function getHrInfo() {
		$.ajax({
			url: service.SHOW_HOME_HR_INFO,
			type: "get",
			dataType: "json",
			data: {},
			success: function(response) {
				if (response.success) {
					var taskCount = response.data.taskCount;
					var resign = response.data.resignInfo;
					$('#resignRate').html(resign.resignRate.toFixed());
					if (resign.resignCount) {
						$('#resignCount').html(resign.resignCount);
					}
					if (taskCount) {
						$('#hrTask').html('待办-调岗审批：' + taskCount);
					} else {
						$('#hrTask').hide();
					}
				} else {
