...
class Car
{
    ...
    public function addToPlan ($date, $planId) {
        ...
        $this->car->type = $exist->car_type;
        $this->car->cold_resistant = $exist->cold_resistant;
        $this->car->assembly_line = $exist->assembly_line;
        $this->car->remark = $exist->remark;
        $this->car->special_property = $exist->special_property;
        $this->car->save();
    ...
    public function addPbsPlanedCar () {
        $ar = new PbsPlanedCarAR();
        $ar->car_id = $this->car->id;
        $ar->vin = $this->car->vin;

        $carSeries = CarSeriesAR::model()->find('series=?', array($this->car->series));
        $ar->series_id = empty($carSeries) ? 0 : $carSeries->id;

        $carType = CarTypeMapAR::model()->find('series=? AND car_type=?', array($this->car->series, $this->car->type));
        $ar->car_type_map_id = empty($carType) ? 0 : $carType->id;

        $carColor = CarColorMapAR::model()->find('series=? AND color=?', array($this->car->series, $this->car->color));
        $ar->color_map_id = empty($carColor) ? 0 : $carColor->id;

        $ar->special_order = $this->car->special_order;
        $ar->save();
    }
