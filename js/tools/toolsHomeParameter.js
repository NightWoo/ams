$(document).ready(function() {
    ...
    $("#tableToolsList").live("click", function(e) {
        if ($(e.target).html() === "编辑") {
            ...
            $("#textEditParameter").val(siblings[1].innerHTML);
            $("#textEditUnit").val(siblings[2].innerHTML);
    ...
    $("#btnAddConfirm").click(function() {
        if($("#textAddParameter").val()==''||$("#textAddUnit").val()==''){
            alert('参数名或单位为空');
            return false;
        }
        ajaxAdd();
    ...
    function ajaxQuery(pageNumber) {
        $.ajax({
            type: "get",
            dataType: "json",
            url: QUERY_TOOLS_HOME_PARAMETER,
            data: {
                "type": 'selects',
    ...
    function ajaxAdd() {
        $.ajax({
            ...
            data: {
                "type": 'adds',
                "parameter": $("#textAddParameter").val(),
                "unit": $("#textAddUnit").val()
            },
            success: function(response) {
                if (response.success) {
                    ajaxQuery(1);
    ...
    function ajaxEdit() {
        $.ajax({
            ...
            data: {
                ...
                "parameter": $("#textEditParameter").val(),
                "unit": $("#textEditUnit").val()
            },
            success: function(response) {
                if (response.success) {
                    ajaxQuery(1);
    ...
    function emptyNewModal() {
        $("#textAddParameter").val("");
        $("#textAddUnit").val("");
    }
    //函数：清空编辑表单
    function emptyEditModel() {
        $("#textEditParameter").val("");
        $("#textEditUnit").val("");
    }
